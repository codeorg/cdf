...
$('#dropzone').droppable({
    ...
    drop: function (e, ui) {
        ...
        //var $el2=$('<div class="drop-item"><div class="form-horizontal"><div class="form-group"><label class="col-sm-2"> <input class="form-control ctrl-dsc" name="co_des" value="Text">        <input name="co_type" type="hidden" value="text">    </label>        <DIV class=col-sm-7><INPUT class=form-control placeholder="文本框"> </DIV></DIV></div><div class="divEdit1"><table class="table table-bordered"><tr><td width="20%">描述</td><td width="35%">值</td><td width="45%">错误提示</td></tr><tr><td>name</td><td><input name="co_name" class="form-control" value="name150446"></td><td>N/A</td></tr><tr><td>是否必须</td><td><input  name="co_required" type="checkbox" ></td><td><input  name="msg_required" class="form-control" placeholder="您输入的不能为空" value="您输入的不能为空"></td></tr><tr><td>最少长度</td><td><input  name="co_minlength"  class="form-control" value=""></td><td><input name="msg_minlength" class="form-control" placeholder="不能少于{0}位" value="不能少于{0}位"></td></tr><tr><td>最大长度</td><td><input name="co_maxlength" class="form-control" value=""></td><td><input name="msg_maxlength" class="form-control" placeholder="不能大于{0}位" value="不能大于{0}位"></td></tr><tr><td>正则表达式</td><td><input name="co_pattern" class="form-control" value="/^[A-Za-z0-9]+$/"></td><td><input name="msg_pattern" class="form-control" value="必须是：A-Za-z0-9"></td></tr><tr><td>失去焦点事件<br>主要验证Ajax回调<br>如：用户名是否存在</td><td><input name="co_blur" class="form-control" value="" placeholder="onblur（ng-blur）事件名"><textarea name="co_blur_function" rows="3" class="form-control text-func" placeholder="$scope.checkUser=function(){$scope.isPost=true;//代码...}"></textarea></td><td><input name="msg_blur" class="form-control" placeholder="如：用户名已经存在"></td></tr></table></div>')
        var $el = $('<div class="drop-item"></div>' );
        var _type=ui.draggable.text();

        _type=_type.replace(/[^a-z]/gi,"");
        _type= $.trim(_type);
        //_type=_type.replace('拖动','').trim();
        var strC=$("<div></div>").append(ui.draggable.find(".col-sm-5").clone()).html().replace('col-sm-5','col-sm-7');
        strC= $.trim(strC)
    ...
    for(var item in obj) {
        switch (item)
        {
            ...
            case "co_blur":
                ...
                errs.blur=$.trim(obj.co_blur).indexOf("()")===-1?' ng-blur="'+$.trim(obj.co_blur)+'()"':' ng-blur="'+$.trim(obj.co_blur)+'"';
                continue;
            case "co_click":
                if(!obj[item]) continue;

                errs.click=$.trim(obj.co_click).indexOf("()")===-1?' ng-click="'+$.trim(obj.co_click)+'()"':' ng-click="'+$.trim(obj.co_click)+'"';
                continue;
            case "co_type":
                if(!obj[item]) continue;
